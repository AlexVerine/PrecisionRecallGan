''' Calculate Inception Features
 ...
 activations of the Inception net (needed for PR Curve).
 '''
import numpy as np
import torch
...
def prepare_parser():
  usage = 'Calculate and store inception features.'
  ...
  with torch.no_grad():
    for i, (x, _) in enumerate(tqdm(loaders[0])):
      ...
      for j in range(x.size(0)):
        ...
        total +=1
  print(imgs[0].shape)
  print(f'Evaluating {len(imgs)} images with inception.')
  ipr = precision_recall_simon_utils.PRCurves(batch_size=64,  num_samples=config['num_pr_images'])
  path = 'samples/features/'+config['dataset'].strip('_hdf5')+'_inception_features.npy'
  ipr.dataset.precomputeFeatures_ref(path, imgs)
